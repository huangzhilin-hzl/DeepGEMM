...
def _check_mxfp4_processed_decode_mapping() -> None:
    """Prove each N128 math-WG slice covers its B tile exactly once.
    ...
    sparse lane error; this contract checks both BN128/1WG and BN256/2WG.
    """
    ...
    def check(block_n: int, num_math_wgs: int) -> None:
        block_k = 128
        wg_block_n = 128
        packed_words_per_row = block_k // 8
        rows_per_decode_group = 8
        assert block_n == num_math_wgs * wg_block_n
        ...
        source_bytes = []
        destination_bytes = []
        per_math_warp = Counter()

        for math_wg_idx in range(num_math_wgs):
            wg_n_idx = math_wg_idx * wg_block_n
            for warp_idx_in_wg in range(4):
                decoder_row_base = warp_idx_in_wg * 32
                for lane_idx in range(32):
                    ...
                    local_scale_owner_n = decoder_row_base + lane_idx
                    scale_word_owners.append(
                        wg_n_idx + local_scale_owner_n)
                    for row_group in range(32 // rows_per_decode_group):
                        ...
                        decoded_n = wg_n_idx + decoded_local_n
                        assert (
                            local_scale_owner_n - lane_idx + row_in_warp ==
                            decoded_local_n)
                        for k32_idx in range(block_k // 32):
                            packed_k = k32_idx * 4 + packed_k_in_k32
                            assert packed_k // 4 == k32_idx
                            visits.append((decoded_n, packed_k))
                            scale_uses.append((decoded_n, k32_idx))
                            per_math_warp[(math_wg_idx, warp_idx_in_wg)] += 1

                            source_row_base = decoded_n * (block_k // 2)
                            logical_source = source_row_base + packed_k * 4
                            physical_source = swizzle(logical_source, bits=2)
                            source_row_xor = (
                                swizzle(source_row_base, bits=2) ^
                                source_row_base)
                            device_physical_source = (
                                source_row_base +
                                ((packed_k * 4) ^ source_row_xor))
                            assert device_physical_source == physical_source
                            source_bytes.extend(range(
                                physical_source, physical_source + 4))

                            logical_destination = (
                                decoded_n * block_k + packed_k * 8)
        ...
        assert set(per_math_warp.values()) == {512}
        assert len(per_math_warp) == num_math_wgs * 4
    ...
    check(block_n=128, num_math_wgs=1)
